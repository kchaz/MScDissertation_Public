...
def global_prompt():
    print("\nGlobal settings: Choose view mode, output mode and model. \n")
    print("Note: to change these, quit iteractive prompt and start over") #make it so can restart
    
    #by journal or overall?
    message = "VIEW MODE: Would you like to look at top documents by journal or overall?"
    message += "\n (0: overall, 1: by journal)"
    choice = int(input(message))
    while choice not in [0,1]:
        print("Invalid value: must be 0 or 1. Please try again.")
        choice = int(input(message))
...
def look_at_topic(choice, latex, k, topn):
    ...
    if choice == 0:
        ...
        out = LdaOutputDocs.get_topic_topdoc_by_journal(df = TM_data_final,
